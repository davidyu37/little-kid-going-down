...
class Population {
  ...
  naturalSelection() {
    ...
    if (
      this.gensSinceNewWorld >= 0 ||
      this.bestScore > (grounds[0].distance - 350) / 10
    ) {
      this.gensSinceNewWorld = 0;
    ...
    console.log(
      "LOOOK HERE THERE ARE " + this.players.length + " Players in this gen"
    );
  }
  ...
  if (contact.GetFixtureA().GetBody().GetUserData().id == "head") {
    if (contact.GetFixtureB().GetBody().GetUserData().id == "ground") {
      ...
      let car = contact
        ...
        .GetJointList()
        .other.GetJointList()
        .other.GetUserData(); //i think that is the grossest code i have ever written
...
function keyPressed() {
  ...
  switch (keyCode) {
    ...
    case RIGHT_ARROW: //right is used to move through the generations
      ...
      } else if (humanPlaying || fightMode) {
